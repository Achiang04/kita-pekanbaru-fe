...
import { ICharacteristicCase } from "../../@types/characteristic";

export default function MultipleSelectCharacteristic({
  field,
  onChange,
  values,
  displayLimit,
  ...
  const onInput = (caseId: number, e: ChangeEvent<HTMLInputElement>) => {
    const value =
      characteristic.id in values.props &&
      Array.isArray(values.props[characteristic.id])
        ? [...values.props[characteristic.id]]
        : [];
    const index = value.findIndex(
      (item: string | number) => String(item) == String(caseId)
    );
  ...
  const isChecked = (caseId: number): boolean => {
    ...
    if (
      characteristicId in values.props &&
      Array.isArray(values.props[characteristicId])
    ) {
      return values.props[characteristicId].some(
        (value: string | number) => String(value) == String(caseId)
      );
    }
      ...
      {collapsedCases.length > 0 && (
        <>
          <Collapse in={showMore} key={characteristic.id}>
            <div>
              {" "}
              {/* Intentional for smooth Collapse animation */}
              <div className="mt-1">
                <CharacteristicCases
