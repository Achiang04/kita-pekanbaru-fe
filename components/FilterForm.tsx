...
export default function FilterForm({
  ...
  const [initedForCategory, setInitedForCategory] = useState<number | null>(
    null
  );
  ...
  const getData = () => {
    ...
    fetchRanges(filterFields, { category: [categoryId], ...sanitizedQuery })
      .then(({ ranges }) => {
        setValues({
          ...sanitizedQuery,
          ...makeInitialValues(ranges, sanitizedQuery),
        });
        setRanges(ranges);
        setIsFetching(false);
      })
  ...
  useEffect(() => {
    const clearedQueryParams = filterEmptyValues(
      filterProductsQuery(queryParams, filterKeys)
    );
    const clearedValues = filterEmptyValues(values);

    if (
      isFilterQueryChanged(clearedQueryParams, clearedValues) ||
      initedForCategory !== categoryId
    ) {
      getData();
    }
  ...
  const reCalcRanges = useCallback(
    _debounce((values) => {
      setIsFetching(true);
      fetchRanges(filterFields, { category: [categoryId], ...values })
        .then(({ ranges, totalProducts }) => {
          setRanges(ranges);
  ...
  const onClear = (e: SyntheticEvent) => {
    ...
    setValues({
      ...clearedValues,
      ...makeInitialValues(ranges, clearedValues),
    });
    setIsFetching(true);
    fetchRanges(filterFields, clearedValues)
      .then(({ ranges }) => {
        setRanges(ranges);
        setHasChanged(false);
        setIsFetching(false);
    ...
    <form className={"category-filters px-1"} onSubmit={onSubmit}>
      ...
      <div className="category-filters__actions">
        <div className="btn-group" role="group">
          ...
          >
            {getSubmitLabel(hasChanged, isFetching, preSearchResult)}
          </button>
        </div>
      </div>
    </form>
...
const fetchRanges = async (
  ...
  const filter_fields = filterFields.map(({ type, characteristic_id }) => ({
    type,
    characteristic_id,
  }));
...
const getSubmitLabel = (
  hasChanged: boolean,
  isFetching: boolean,
  preSearchResult: null | number
): string => {
  if (hasChanged) {
    if (isFetching) return "Calculating...";

    if (preSearchResult !== null) return `Show (${preSearchResult})`;
  }
...
const makeInitialValues = (
  filterFields: IFilterFieldRange[],
  query: TQuery
) => {
  const out: TQuery = {};
    ...
    switch (filterField.type) {
      case TFilterFieldType.price:
        Object.assign(
          out,
          { price_min: "", price_max: "" },
          _pick(query, ["price_min", "price_max"])
        );
        break;

      case TFilterFieldType.brand:
        Object.assign(out, {
          brand: (query.brand || []).map((el: string) => Number(el)),
        });
        break;
      ...
      case TFilterFieldType.characteristic: {
        const props =
          _isObjectLike(query.props) && !Array.isArray(query.props)
            ? query.props
            : {};
        if (!("props" in out)) {
          out.props = {};
        ...
        if (isMultiCaseType(filterField.characteristic!.type)) {
          out.props[filterField.characteristic_id!] =
            filterField.characteristic_id! in props &&
            Array.isArray(props[filterField.characteristic_id!])
...
const isFilterQueryChanged = (prevQuery: TQuery, newQuery: TQuery): boolean => {
  ...
  for (const key of filterKeys) {
    if (!(key in prevQuery) && !(key in newQuery)) continue;
...
const isMultiCaseType = (type: TCharacteristicType) =>
  [
    TCharacteristicType.radio,
    TCharacteristicType.select,
    TCharacteristicType.checkbox,
  ].includes(type);
