...
import { IVariant, IVariantIdCombinations } from "../../../@types/variant";
import { IVariantCharacteristic } from "../../../@types/characteristic";

// values, onSelect, idCombinations
export default function VariantPickerCharacteristic({
  characteristic,
  onSelectCase,
  value,
  idCombinations,
  ...
  const onLabelClicked = (caseId: number, e: MouseEvent<HTMLLabelElement>) => {
    ...
    onSelectCase(
      characteristic.id,
      value[characteristic.id] === caseId ? null : caseId
    );
  };
  ...
  return (
    <div className={"variant-picker__characteristic"}>
      <div
        className={"variant-picker__title"}
      >{`${characteristic.title}:`}</div>
      <div className="variant-picker__cases">
        {characteristic.cases.map((caseItem) => {
          const id = `${characteristic.id}-case-${caseItem.id}`;
          const availableVariants = findAvailableVariants(
            variants,
            idCombinations,
            { ...value, ...{ [characteristic.id]: caseItem.id } }
          );
          const inStockVariants = availableVariants.filter(
            ({ in_stock }) => in_stock
          );

          return (
            <div key={caseItem.id} className={"variant-picker__case-item"}>
              <input
                autoComplete={"off"}
                className={"btn-check"}
              ...
              <label
                className={clsx("btn btn-outline-secondary", {
                  "out-of-stock": !inStockVariants.length,
                })}
                htmlFor={id}
...
const isValueSuitsCombination = (
  combination: { [characteristicId: number | string]: number },
  value: { [characteristicId: number | string]: number }
): boolean => {
  for (const [characteristicId, caseId] of Object.entries(value)) {
    if (
      !(characteristicId in combination) ||
      combination[characteristicId] != caseId
    ) {
      return false;
    }
  }
